<template>
    <div class="orderstate">
      <div class="codeDebug">{{getOrderstateData?'':''}}</div>
      <div class="codeDebug">{{getOrderstateInfoData?'':''}}</div>
      ...
      <div class="listWrap">
        ...
        <div class="cntJiluWrap">
          ...
          <div class="cntJiluLineBox">
            ...
            <div class="circleBox">
              ...
              <div class="circleItem" v-on:click="toState($event)" v-for="circleitem in orderstateList" :key="circleitem.id + 'circle'" v-if="circleitem.tongguo !== 'tongguo'" v-bind:class="circleitem.circleMove?'circleMove':''"></div>
        ...
        <div class="progressItemWrap">
          <form class="myform" enctype="multipart/form-data" action="" method="post">
            <div class="progressTopBox">
              ...
              <div class="progressUpBtn">
                <div class="progressIptBox">
                  ...
                  <div class="progressEndDate"><span class="lastdate">已通过</span></div>
                </div>
              </div>
              <div class="progressUpBtnReal" v-on:click="imgUpload($event)">
                <!--<div class="progressUpBtnRealPic"><img src="../assets/imgUploadIcon.png" /></div>-->
              </div>
            </div>
            <div class="yincangyu"><input type="text" name="billId" class="billId" id="billId" value="115" /></div>
            <div class="yincangyu"><input type="text" name="status" class="status" id="status" value="BL" /></div>
            ...
            <!--<div class="subbtn" v-on:click="formSub()">提交</div>-->
          </form>
        </div>
...
export default {
    ...
    data () {
        return {
            ...
            imgUploadPayload: {
                key: 'imgUpload',
                // queryOrderBillsByStatus queryOrderBillsByStatusForWeb
                url: this.$store.state.baseServiceUrl + '/upload/uploadImgForOrderProcedure',
                postData: {
                    billId: this.orderid
                }
            }
    ...
    created () {
        ...
        this.itemindex = parseInt(that.GetQueryString('itemindex'))
    ...
    methods: {
        ...
        formSub: function () {
            ...
            if (this.imgBase64) {
                ...
                $('.coverBox').fadeIn()
                $('.myform').attr('action', baseUrl + '/upload/uploadImgForOrderProcedure')
                $('.myform').submit()
            } else {
                // console.log('imgBase64:' + this.imgBase64)
            }
        },
        currentTitIni: function () {
            var that = this
            switch (parseInt(that.itemindex)) {
            case 0:
            ...
            case 4:
                ...
                break
            default:
                console.log(parseInt(that.itemindex))
            }
        },
        toStateDet: function (e) {
            var obj = e.currentTarget
            var that = this
            that.itemindex = $(obj).index()
            this.$router.push('/OrderstateDet?order=' + this.orderid + '&bill=' + this.billno + '&itemindex=' + that.itemindex)
        },
        toState: function () {
            this.$router.push('/Orderstate?order=' + this.orderid + '&bill=' + this.billno)
        }
    }
}
</script>
...
<style scoped>
  ...
  *{
  ...
  .circleMove{
    animation:circleMove 1.5s linear infinite;
  }
